...
/**
 * Main implementation for Sauce Labs testing
 */
...
public class iSauceBase implements SauceOnDemandSessionIdProvider, SauceOnDemandAuthenticationProvider, UDBase {
    ...
    /**
     * Before any test class the below method gets the test domain for all test methods
     */
    @BeforeClass
     ...
     * @param profileUpload Set to true or false up enable FF profile uploading
     * @param method Test Method
     * @throws Exception
     */
    @Parameters({"username", "key", "os", "browser", "version","profileUpload"})
    @BeforeMethod
    public void setUp(@Optional("") String username,
        ...
        if (StringUtils.isNotBlank(browserProp) && StringUtils.isNotBlank(platformProp)) {
            ...
            // If profile upload is false no profile wil get uploaded
            ...
            // Admin tests have 'create' in it's name. No reason to run the profile with them
            // Also check if profileUpload equals true
